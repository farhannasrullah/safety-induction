...
export default function App() {
  ...
  const [deleteData, setDeleteData] = useState(null); // State untuk modal delete
  ...
  useEffect(() => {
    if (isAdminLoggedIn) {
      ...
      if (loginTime && now - parseInt(loginTime) > ONE_HOUR) {
        // Jika sudah lebih dari 1 jam saat di-refresh
        handleAdminLogout();
        showError("Sesi Admin telah berakhir (Timeout 1 Jam). Silakan login kembali.");
      } else {
        // Set timer untuk sisa waktu menuju 1 jam
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      setUser(currentUser);
      setIsAppLoading(false); // Matikan loading saat status auth sudah diketahui
    });
    return () => unsubscribe();
  }, []);
  ...
  useEffect(() => {
    if (viewMode === 'admin' && isAdminLoggedIn && user) {
      ...
      const unsubscribe = onSnapshot(inductionsRef, (snapshot) => {
        ...
        setAdminData(data);
        setIsAdminLoading(false); // Matikan spinner tabel
      });
      return () => unsubscribe();
    }
  ...
  const validateForm = () => {
    if (!formData.nama || !formData.noPribadi || !formData.instansi || !formData.posisi || !formData.kontakDarurat || !formData.hubunganKontak) {
      showError("Harap lengkapi semua kolom identitas, pekerjaan, dan kontak darurat Anda.");
      return false;
    }
    setErrorMsg("");
    setStep(2);
  ...
  const handleVideoSeeking = (e) => {
    ...
    if (video.currentTime > maxTimeWatched.current + 1) {
      ...
      showError("Peringatan: Anda tidak dapat mempercepat/melewati video materi induksi.");
    }
  };

  // --- QUIZ LOGIC (DENGAN REMEDIAL) ---
  ...
  const validateQuiz = () => {
    ...
    if (!allAnswered) {
      showError("Harap jawab semua pertanyaan evaluasi (5 soal) terlebih dahulu.");
      return;
    }
    
    if (errors.length > 0) {
      setWrongAnswers(errors);
      showError(`Terdapat ${errors.length} jawaban yang keliru. Silakan periksa kotak merah dan perbaiki jawaban Anda.`);
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const inductionsRef = collection(db, 'artifacts', appId, 'public', 'data', 'inductions');
      // Tangkap document reference untuk mendapatkan ID unik
      ...
      // Kirim Data dengan Action 'create' dan sertakan ID Firestore
      await fetch(GAS_URL, {
        method: "POST",
        mode: "no-cors", 
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: 'create', // Action untuk Apps Script
          id: docRef.id,    // Simpan ID ini di Sheets untuk keperluan Edit/Delete nanti
          nama: formData.nama,
      ...
      console.error(err);
      showError("Terjadi kesalahan. Pastikan koneksi internet stabil.");
    } finally {
  ...
  const handleAdminLogin = (e) => {
    ...
    if (loginForm.username === 'admin' && loginForm.password === 'admin123') {
      ...
      sessionStorage.setItem('admin_login_time', Date.now().toString()); // Catat waktu login
      setLoginError(""); 
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      console.error(e);
      showError("Gagal menghapus data dari sistem.");
    }
  };
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center font-sans text-slate-800 relative overflow-hidden">
        ...
        <div className="flex flex-col items-center z-10 animate-fade-in-up">
          ...
          <h2 className="mt-8 text-xl font-extrabold text-slate-900 tracking-widest flex items-center gap-2">
            MEMUAT SISTEM
          </h2>
  ...
  return (
    <>
      ...
      <div className="min-h-screen font-sans text-slate-800 selection:bg-yellow-400 selection:text-slate-900 bg-slate-50 flex flex-col relative">
        ...
        {/* Production-Level Warning Notification (Z-INDEX TERTINGGI, FIXED) */}
        {errorMsg && (
          <div className="fixed top-4 left-0 right-0 z-[99999] animate-slide-down flex justify-center pointer-events-none px-4">
            <div className="bg-red-500/95 backdrop-blur-md text-white px-5 py-3.5 rounded-2xl shadow-2xl flex items-center gap-3 border border-red-400 w-auto max-w-xl mx-auto ring-4 ring-red-500/20">
        ...
        {/* Header Fixed - Benar-benar menempel di atas */}
          ...
          <div className={`mx-auto p-4 flex items-center justify-between gap-3 transition-all ${viewMode === 'admin' && isAdminLoggedIn ? 'max-w-7xl' : (viewMode === 'user' ? 'max-w-5xl' : 'max-w-md')}`}>
            ...
            {viewMode === 'admin' && (
              <div className="flex gap-2">
                {isAdminLoggedIn && (
                  <button onClick={handleAdminLogout} className="flex items-center gap-2 text-sm font-bold text-red-600 hover:text-red-700 bg-red-50 hover:bg-red-100 px-4 py-2 rounded-lg transition-colors border border-red-100">
                    <LogOut className="w-4 h-4 hidden sm:block" /> Keluar
                  </button>
                )}
                {!isAdminLoggedIn && (
        ...
        {/* Main Wrapper dengan Padding Top untuk menyeimbangkan Header yang Fixed */}
        <div className="relative z-10 flex flex-col min-h-screen w-full pt-20 md:pt-24">
          <main className={`mx-auto p-4 pb-8 relative flex-grow w-full transition-all ${viewMode === 'admin' && isAdminLoggedIn ? 'max-w-7xl' : (viewMode === 'user' ? 'max-w-5xl' : 'max-w-md')}`}>
                        ...
                        {isAdminLoading ? (
                          ...
                          getProcessedAdminData().map((item) => (
                            <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group/row">
                              ...
                              <td className="p-4 text-center">
                                {/* Tombol Aksi CRUD Selalu Terlihat Tanpa Hover */}
            ...
            {/* ========================================================= */}
            {/* VIEW: USER MODE (FORM WIZARD DENGAN 5 STEPS EDUKASI) */}
            {/* ========================================================= */}
            {viewMode === 'user' && (
              <>
                {step !== 6 && renderProgressBar()}

                {/* STEP 1: Form Pendaftaran (Horizontal on MD) */}
                {step === 1 && (
                ...
                {/* STEP 2: Video Induksi (Center Block) */}
                {step === 2 && (
                  <div className="bg-white rounded-3xl shadow-xl p-8 animate-fade-in-up border border-gray-100 max-w-3xl mx-auto">
                    ...
                    <div className="flex gap-4 flex-col sm:flex-row">
                      ...
                      <button onClick={() => { if (isVideoFinished) setStep(3); else showError("Anda harus menonton video sampai durasi selesai."); }} disabled={!isVideoFinished} className={`flex-1 font-bold py-4 rounded-2xl flex items-center justify-center gap-3 transition-all duration-300 ${isVideoFinished ? 'bg-yellow-400 hover:bg-yellow-500 text-slate-900 shadow-lg shadow-yellow-400/30 btn-pulse-glow transform hover:-translate-y-1' : 'bg-gray-100 text-gray-400 cursor-not-allowed border-2 border-dashed border-gray-200'}`}>
                ...
                {/* STEP 3: Poster Keselamatan (Horizontal on MD - Disesuaikan & Difullkan) */}
                {step === 3 && (
                  <div className="bg-white rounded-3xl shadow-xl animate-fade-in-up border border-gray-100 overflow-hidden flex flex-col md:flex-row max-w-6xl mx-auto min-h-[600px]">
                    ...
                    {/* Panel Teks/Instruksi - Di-full-kan height-nya */}
                    <div className="md:w-5/12 bg-slate-900 border-r border-gray-100 relative overflow-hidden flex flex-col justify-center p-8 md:p-12">
                      ...
                      <h3 className="text-white font-extrabold text-3xl flex items-center gap-3 mb-4">
                        <Info className="w-8 h-8 text-yellow-400"/> Poster<br/>Keselamatan
                      </h3>
                    ...
                    {/* Panel Poster Center - Dibuat full menyesuaikan gambar */}
                    <div className="md:w-7/12 p-8 flex flex-col bg-slate-50 relative">
                      <div className="flex-grow w-full flex items-center justify-center mb-8 relative rounded-2xl bg-white border border-gray-200 shadow-inner p-2 md:p-4">
                        <img 
                          ...
                          alt="Poster K3" 
                          className="w-full h-full max-h-[60vh] object-contain rounded-xl drop-shadow-sm"
                        />
                      </div>
                      ...
                      <div className="flex gap-4 w-full mt-auto">
                        ...
                        <button onClick={() => setStep(4)} className="w-2/3 bg-slate-900 hover:bg-slate-800 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 shadow-lg transition-all transform hover:-translate-y-1 hover:shadow-slate-900/20 group">
                          Paham & Lanjut Kuis <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform"/>
                        </button>
                      </div>
                    </div>
                ...
                {/* STEP 4: Evaluasi Kuis (Center Block) */}
                {step === 4 && (
                  <div className="bg-white rounded-3xl shadow-xl p-6 md:p-10 animate-fade-in-up border border-gray-100 max-w-3xl mx-auto flex flex-col">
                        ...
                        return (
                          <div key={item.id} className={`p-6 rounded-2xl border transition-all ${isWrong ? 'bg-red-50/50 border-red-300 ring-2 ring-red-100 shadow-sm' : 'bg-slate-50 border-slate-200'}`}>
                            <h3 className="font-bold text-slate-800 text-[15px] mb-5 leading-relaxed flex items-start gap-3">
                              <span className={`flex-shrink-0 w-7 h-7 inline-flex items-center justify-center rounded-full text-xs font-black shadow-sm ${isWrong ? 'bg-red-500 text-white' : 'bg-yellow-400 text-slate-900'}`}>
                                {qIndex + 1}
                              </span>
                                ...
                                return (
                                  <label key={optIndex} className={`flex items-start gap-3 p-3.5 rounded-xl cursor-pointer transition-all border-2 ${isSelected ? (isWrong ? 'bg-red-50 border-red-400' : 'bg-white border-yellow-400 shadow-md ring-2 ring-yellow-400/20') : 'bg-white border-gray-100 hover:border-yellow-200 hover:bg-yellow-50/30'}`}>
                                    <input 
                                      ...
                                      className={`mt-0.5 w-4 h-4 bg-gray-100 border-gray-300 focus:ring-yellow-500 cursor-pointer ${isWrong ? 'accent-red-500' : 'accent-yellow-500'}`}
                                      checked={isSelected}
                    ...
                    <div className="flex gap-4 border-t border-gray-100 pt-8 mt-auto">
                      ...
                      <button onClick={validateQuiz} className="w-2/3 bg-slate-900 hover:bg-slate-800 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 transition-all shadow-lg hover:shadow-slate-900/20 group">
                        Cek & Pengesahan <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                      </button>
                    </div>
                  </div>
                )}

                {/* STEP 5: E-Signature (Horizontal on MD) */}
                {step === 5 && (
                  <div className="bg-white rounded-3xl shadow-xl animate-fade-in-up border border-gray-100 overflow-hidden flex flex-col md:flex-row max-w-5xl mx-auto">
                    
                    {/* Panel Kiri: Komitmen T&C */}
                    <div className="md:w-1/2 p-8 md:p-12 bg-slate-50 border-r border-gray-100 flex flex-col h-full">
                      ...
                      <div className="bg-white border border-slate-200 p-6 rounded-2xl text-sm text-slate-600 shadow-sm flex-grow flex flex-col justify-center">
                        ...
                        <p className="text-justify leading-relaxed mb-4">
                          Saya <strong>{formData.nama}</strong> ({formData.posisi} - {formData.instansi}) berkomitmen untuk:
                        </p>
                ...
                {/* STEP 6: Success (Diperkecil / Max-w-lg) */}
                {step === 6 && (
                  <div className="bg-white rounded-3xl shadow-xl p-8 text-center animate-fade-in-up border border-gray-100 relative overflow-hidden max-w-lg mx-auto">
                    ...
                    <div className="relative z-10">
                      ...
                      <button 
                        onClick={() => {
                          sessionStorage.clear(); // Hapus seluruh histori form lama
                          window.location.reload(); // Segarkan bersih dari awal
                        }} 
                        className="bg-slate-900 text-white font-bold py-4 px-8 rounded-2xl w-full hover:bg-slate-800 transition-colors shadow-xl flex items-center justify-center gap-3"
                      >
        ...
        {/* ========================================================= */}
        {/* STANDALONE CUSTOM MODAL: PREVIEW SIGNATURE (Z-INDEX TERTINGGI) */}
        {/* ========================================================= */}
        {viewMode === 'admin' && selectedSignature && (
          <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6" style={{ backgroundColor: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(8px)' }}>
            ...
            <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-2xl animate-fade-in-up overflow-hidden flex flex-col border border-slate-200">
              <div className="flex justify-between items-center p-5 sm:p-6 border-b border-gray-100 bg-slate-50/80">
                ...
                <button onClick={() => setSelectedSignature(null)} className="text-slate-400 hover:text-red-500 hover:bg-red-50 p-2 rounded-xl transition-colors">
              ...
              <div className="p-5 sm:p-6 border-t border-gray-100 bg-white">
                <button onClick={() => setSelectedSignature(null)} className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-4 rounded-2xl transition-all shadow-lg hover:shadow-slate-900/20 active:scale-[0.98]">
                  Tutup Pratinjau
                </button>
              </div>
            </div>
        ...
        {/* ========================================================= */}
        {/* STANDALONE CUSTOM MODAL: EDIT DATA ADMIN (Z-INDEX TERTINGGI) */}
        {/* ========================================================= */}
        {viewMode === 'admin' && isEditModalOpen && editData && (
          <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6" style={{ backgroundColor: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(8px)' }}>
            <div className="absolute inset-0 cursor-pointer" onClick={() => setIsEditModalOpen(false)}></div>
            <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-2xl animate-fade-in-up overflow-hidden flex flex-col border border-slate-200 max-h-[90vh]">
              <div className="flex justify-between items-center p-5 border-b border-gray-100 bg-slate-50/80">
                ...
                <button onClick={() => setIsEditModalOpen(false)} className="text-slate-400 hover:text-red-500 hover:bg-red-50 p-2 rounded-xl transition-colors">
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <div className="p-5 border-t border-gray-100 bg-slate-50 flex gap-3 justify-end">
                <button type="button" onClick={() => setIsEditModalOpen(false)} className="px-6 py-2.5 font-bold text-slate-600 bg-white border border-gray-200 hover:bg-gray-50 rounded-xl transition-colors">Batal</button>
                <button type="submit" form="editForm" className="px-6 py-2.5 font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition-colors shadow-lg hover:shadow-blue-600/20 flex items-center gap-2">
        ...
        {/* ========================================================= */}
        {/* STANDALONE CUSTOM MODAL: KONFIRMASI HAPUS DATA ADMIN */}
        {/* ========================================================= */}
        {viewMode === 'admin' && deleteData && (
          <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6" style={{ backgroundColor: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(8px)' }}>
            <div className="absolute inset-0 cursor-pointer" onClick={() => setDeleteData(null)}></div>
            <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm animate-fade-in-up overflow-hidden flex flex-col border border-slate-200 text-center p-8">
              ...
              <p className="text-sm text-slate-500 mb-8 leading-relaxed">
                Anda yakin ingin menghapus data induksi atas nama <strong className="text-slate-800">{deleteData.nama}</strong>? Tindakan ini tidak dapat dibatalkan.
              </p>
              <div className="flex gap-3 w-full">
                <button onClick={() => setDeleteData(null)} className="flex-1 bg-gray-100 hover:bg-gray-200 text-slate-700 font-bold py-3.5 rounded-2xl transition-colors">
                  Batal
                </button>
                <button onClick={executeDelete} className="flex-1 bg-red-500 hover:bg-red-600 text-white font-bold py-3.5 rounded-2xl transition-colors shadow-lg hover:shadow-red-500/20">
                  Ya, Hapus
                </button>
              </div>
            </div>
